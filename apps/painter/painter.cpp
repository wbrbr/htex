...
{
    bool success = true;
    if (argc != 2) success = false;

    if (!success) {
        fprintf(stderr, "Usage: %s <ccm file>\n", argv[0]);
    }

    g_state.shadingMode = SHADING_BASECOLOR;

    if (success) {
        g_htex.mesh = ccm_Load(argv[1]);
        if (!g_htex.mesh) success = false;
    }

    const int log_res = 4;
    std::vector<uint8_t> texels(4*(1<<log_res)*(1<<log_res));
    for (int i = 0; i < 1 << log_res; i++) {
        for (int j = 0; j < 1 << log_res; j++) {
            int px = i*(1<<log_res) + j;
            texels[4*px+0] = 0x55;
            texels[4*px+1] = 0x55;
            texels[4*px+2] = 0x55;
            texels[4*px+3] = 0xff;
    ...
    g_htex.quadLog2Resolutions.resize(2*g_htex.mesh->edgeCount);
    for (int quadID = 0; quadID < g_htex.mesh->edgeCount; quadID++) {
        g_htex.quadLog2Resolutions[2*quadID+0] = log_res;
        g_htex.quadLog2Resolutions[2*quadID+1] = log_res;
    }

    g_htex.textureFlags |= (1 << TEXTURETYPE_COLOR);
    g_gl.edgeTextures.resize(TEXTURETYPE_COUNT*g_htex.mesh->edgeCount);
    int offset = TEXTURETYPE_COLOR*g_htex.mesh->edgeCount;
    glCreateTextures(GL_TEXTURE_2D, g_htex.mesh->edgeCount, &g_gl.edgeTextures[offset]);
    for (int quadID = 0; quadID < g_htex.mesh->edgeCount; quadID++) {
        int tex = g_gl.edgeTextures[offset+quadID];
        glTextureStorage2D(tex, 1, GL_RGBA8, 1 << log_res, 1 << log_res);
        glTextureSubImage2D(tex, 0, 0, 0, 1 << log_res, 1 << log_res, GL_RGBA, GL_UNSIGNED_BYTE, texels.data());
